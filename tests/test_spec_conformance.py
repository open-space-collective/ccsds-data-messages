...
import pydantic
import pytest
from conftest import CREATION_DATE, EPOCH, FIXTURES, make_oem

from ccsds_data_messages import OCM, OEM, OMM, OPM, read
...
class TestSection77KvnUnits:
    ...
    def test_oem_kvn_ephemeris_data_lines_have_no_inline_units_when_include_units_false(
        ...
        """§7.7.2.1: With include_units=False, ephemeris lines have no [unit] annotations."""
        ...
        for line in data_lines:
            assert "[" not in line, (
                f"§7.7.2.1: inline unit found in OEM ephemeris line (include_units=False): {line!r}"
            )

    def test_oem_kvn_ephemeris_data_lines_have_no_inline_units_when_include_units_true(
        self,
    ):
        """§7.7.2.1: Even with include_units=True, OEM KVN ephemeris lines must not carry units."""
        oem = make_oem(n_lines=3)
        writer = KVNOEMWriter()
        ...
        for line in data_lines:
            assert "[" not in line, (
                f"§7.7.2.1: inline unit found in OEM ephemeris line (include_units=True): {line!r}"
            )
        ...
        for line in output.splitlines():
            ...
            if (
                ...
                assert "[" not in stripped, (
                    f"§7.7.2.2: inline unit in OEM KVN covariance data line: {stripped!r}"
                )
    ...
    def test_ocm_kvn_trajectory_data_lines_have_no_inline_units(self):
        ...
        for line in output.splitlines():
            ...
            if stripped and stripped[0].isdigit() and "T" in stripped.split()[0]:
                assert "[" not in stripped, (
                    f"§7.7.3.5: inline unit in OCM KVN trajectory data line: {stripped!r}"
                )

    def test_kvn_parser_strips_inline_unit_from_key_value_line(self):
        """§7.7.1.1: KVN parser must strip the optional [unit] suffix, not include it in value."""
        from ccsds_data_messages.io.kvn.parser import KeyValueLine, parse_kvn
    ...
    def test_kvn_parser_value_is_correct_after_unit_strip(self):
        """§7.7.1.1: Value parsed from 'X = -1234.5 [km]' is exactly '-1234.5'."""
        from ccsds_data_messages.io.kvn.parser import KeyValueLine, parse_kvn
...
class TestSection791ValidVersions:
    ...
    def test_opm_version_3_x_accepted(self, vers: str):
        """OPM versions in {1.0, 2.0, 3.0} must all construct. (§7.9.1)"""
    ...
    def test_opm_version_in_valid_set_accepted(self, vers: str):
        """
        §7.9.1 lists OPM versions as {1.0, 2.0, 3.0}; any of these must construct.
    ...
    def test_opm_version_4_0_raises_outside_valid_set(self):
        """§7.9.1: '4.0' is not a valid OPM version; must raise ValidationError."""
        with pytest.raises(pydantic.ValidationError):
            OPM.Header(ccsds_opm_vers="4.0", **_HEADER_KW)

    def test_opm_version_non_numeric_raises(self):
        """§7.9.1: non-numeric version strings must raise ValidationError."""
        with pytest.raises(pydantic.ValidationError):
    ...
    def test_omm_version_2_0_is_valid_per_spec(self):
        """
        §7.9.1 lists OMM versions as {2.0, 3.0}; v2.0 must construct.
    ...
    def test_omm_version_1_0_raises_not_in_valid_set(self):
        """§7.9.1: OMM has no version 1.0; must raise ValidationError."""
        with pytest.raises(pydantic.ValidationError):
            OMM.Header(ccsds_omm_vers="1.0", **_HEADER_KW)

    def test_omm_version_4_0_raises_outside_valid_set(self):
        """§7.9.1: '4.0' is not a valid OMM version; must raise ValidationError."""
        with pytest.raises(pydantic.ValidationError):
    ...
    def test_oem_version_in_valid_set_accepted(self, vers: str):
        """
        §7.9.1 lists OEM versions as {1.0, 2.0, 3.0}; any must construct.
    ...
    def test_oem_version_4_0_raises_outside_valid_set(self):
        """§7.9.1: '4.0' is not a valid OEM version; must raise ValidationError."""
        with pytest.raises(pydantic.ValidationError):
...
class TestSection513OEMSingleObject:
    ...
    def test_oem_segments_with_different_object_names_raises_validation_error(self):
        """§5.1.3: segments with different OBJECT_NAME values violate the single-object rule."""
...
class TestSection5247InterpolationRecords:
    ...
    def test_lagrange_degree_5_with_2_data_lines_raises_validation_error(self):
        """§5.2.4.7: LAGRANGE degree=5 requires >= 6 records; only 2 must raise."""
    ...
    def test_linear_interpolation_with_1_data_line_raises_validation_error(self):
        """§5.2.4.7: LINEAR requires >= 2 records to interpolate; 1 line must raise."""
        with pytest.raises(pydantic.ValidationError, match="ephemeris data records"):
            OEM(
                header=OEM.Header(ccsds_oem_vers="3.0", **_HEADER_KW),
                segments=[
...
class TestSection4246OMMSGP4:
    ...
    def test_sgp4_theory_with_teme_frame_and_earth_center_accepted(self):
        """§4.2.4.6 happy path: SGP4 + TEME + EARTH + UTC must construct."""
    ...
    def test_sgp4_theory_with_eme2000_frame_raises_validation_error(self):
        """§4.2.4.6: SGP4 requires REF_FRAME=TEME; EME2000 must raise ValidationError."""
    ...
    def test_sgp4_theory_with_gcrf_frame_raises_validation_error(self):
        """§4.2.4.6: SGP4 requires REF_FRAME=TEME; GCRF must also raise ValidationError."""
        with pytest.raises(pydantic.ValidationError, match="requires REF_FRAME=TEME"):
