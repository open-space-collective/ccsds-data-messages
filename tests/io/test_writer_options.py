...
class TestFloatFormats:
    def test_float_formats_override_applies_to_specified_keyword(self):
        """WriterOptions(float_formats={'X': '.15g'}) makes the X line use 15 sig figs."""
...
def _opm_with_user_defined() -> OPM:
    """OPM carrying two USER_DEFINED_x entries with differing suffix lengths."""
    base = make_opm()
    return OPM(
        header=base.header,
        metadata=base.metadata,
        data=OPM.Data(
            state_vector=base.data.state_vector,
            user_defined=OPM.Data.UserDefinedParameters(
                user_defined={"A": "1", "EARTH_MODEL": "WGS-84"},
            ),
        ),
    )


class TestUserDefinedEmission:
    """
    USER_DEFINED_x lines must honor WriterOptions like every other keyword.

    Regression: OPM/OMM previously routed user-defined parameters through a helper
    that ignored options entirely, so USER_DEFINED lines were never keyword-aligned.
    """

    def test_user_defined_lines_align_when_align_keywords_true(self):
        opm = _opm_with_user_defined()
        output = _OPM_WRITER.write_string(opm, options=WriterOptions(align_keywords=True))
        ud_lines = [
            line for line in output.splitlines() if line.startswith("USER_DEFINED_")
        ]
        assert len(ud_lines) == 2
        eq_positions = [line.index("=") for line in ud_lines]
        assert len(set(eq_positions)) == 1, (
            f"USER_DEFINED '=' should align; positions: {eq_positions}"
        )

    def test_user_defined_lines_compact_when_align_keywords_false(self):
        opm = _opm_with_user_defined()
        output = _OPM_WRITER.write_string(
            opm, options=WriterOptions(align_keywords=False)
        )
        ud_lines = [
            line for line in output.splitlines() if line.startswith("USER_DEFINED_")
        ]
        ...
        eq_positions = [line.index("=") for line in ud_lines]
        assert len(set(eq_positions)) > 1, (
            f"Without alignment USER_DEFINED '=' should differ; positions: {eq_positions}"
        )


class TestSuppressDefaultsSentinel:
    """
    Behavior of the ``suppress_defaults=None`` sentinel.

    Makes 'no options' and ``WriterOptions()`` behave identically, and lets each
    writer pick its own per-message-type default.
    """

    def test_ocm_none_and_empty_options_produce_identical_output(self):
        # ocm_g17 omits TIME_SYSTEM (defaults to UTC); the OCM writer suppresses it.
        ocm = read(FIXTURES / "ocm_g17_deployments.kvn", fmt="kvn", message_type="ocm")
        out_none = _OCM_WRITER.write_string(ocm)
        out_empty = _OCM_WRITER.write_string(ocm, options=WriterOptions())
        assert out_none == out_empty
        assert "TIME_SYSTEM" not in out_none

    def test_ocm_explicit_suppress_false_emits_defaulted_field(self):
        ocm = read(FIXTURES / "ocm_g17_deployments.kvn", fmt="kvn", message_type="ocm")
        out = _OCM_WRITER.write_string(
            ocm, options=WriterOptions(suppress_defaults=False)
        )
        assert "TIME_SYSTEM = UTC" in out


class TestIncludeUnits:
    @pytest.mark.xfail(
        reason=(
            "OPM/OMM KVN inline units are spec-optional (section 7.7.1.1) but not yet implemented. "
    ...
    def test_opm_kvn_with_include_units_true_emits_optional_inline_units_per_spec(self):
        """Section 7.7.1.1: OPM/OMM KVN may include [km] etc. after values; include_units=True should emit them."""
        opm = make_opm()
    ...
    def test_oem_kvn_ephemeris_data_lines_never_have_inline_units_regardless_of_option(
        ...
        """
        Section 7.7.2.1: OEM KVN ephemeris data lines must not display units - even with include_units=True.

        Spec quote: 'units shall be km, km/s, and km/s^2 ... but the units shall not be displayed.'
        """
        ...
        for include in (True, False):
            ...
            for line in data_lines:
                assert "[" not in line, (
                    f"section 7.7.2.1: inline unit in OEM ephemeris line (include_units={include}): {line!r}"
                )
        ...
        for line in output.splitlines():
            ...
            if (
                ...
                assert "[" not in stripped, (
                    f"section 7.7.2.2: inline unit in OEM covariance data line: {stripped!r}"
                )
    ...
    def test_ocm_kvn_trajectory_data_lines_never_have_inline_units(self):
        ...
        for line in output.splitlines():
            ...
            if stripped and stripped[0].isdigit() and "T" in stripped.split()[0]:
                assert "[" not in stripped, (
                    f"section 7.7.3.5: inline unit in OCM trajectory data line: {stripped!r}"
                )
    ...
    def test_writer_options_defaults(self):
        ...
        assert opts.suppress_defaults is None


def _ocm_with_propulsive_maneuver() -> OCM:
    """A minimal OCM with one propulsive maneuver row (THR_X = 0.0005)."""
    man = OCM.ManeuverSpecification(
        ...
        data_lines=[
            OCM.ManeuverSpecification.ManeuverDataLine(
                time_tag="2020-001T00:00:00",
                man_dura=60.0,
                thr_x=0.0005,
                thr_y=0.0,
                thr_z=0.0,
...
class TestManeuverColumnFloatFormats:
    """WriterOptions.float_formats controls maneuver data-line column notation."""

    def test_default_uses_adaptive_fixed_point(self):
        # 0.0005 under the default .15g is fixed-point, no exponent.
        out = _OCM_WRITER.write_string(_ocm_with_propulsive_maneuver())
        assert "e-04" not in out
        assert "0.0005" in out

    def test_float_formats_overrides_column_to_scientific(self):
        out = _OCM_WRITER.write_string(
            _ocm_with_propulsive_maneuver(),
            options=WriterOptions(float_formats={"THR_X": " .2e"}),
        )
        assert "5.00e-04" in out  # THR_X forced to floating-point notation
