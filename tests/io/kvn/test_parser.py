...
def test_parse_kvn_kv_line_extra_whitespace_around_equals():
    # Section 7.4.5-7.4.6: whitespace around '=' is not significant
    lines = parse_kvn("X   =   6503.0 [km]")
    assert lines == [KeyValueLine(keyword="X", value="6503.0")]


def test_parse_kvn_covariance_start_block():
    # COVARIANCE_START is a valid block delimiter in OPM/OEM (section 3.2.4.10)
...
def test_parse_kvn_blank_line_in_sequence():
    # Section 7.3.5: blank lines may appear anywhere in a KVN document
    text = "CCSDS_OPM_VERS = 3.0\n\nCREATION_DATE = 2022-11-06T09:23:57\n"
    lines = parse_kvn(text)
...
# ---------------------------------------------------------------------------
# Diagnostic warnings - malformed-but-tolerated input (section 7.3.2, 7.3.4, 7.4.4, 7.8.5)
# ---------------------------------------------------------------------------


def test_parse_kvn_bare_comment_warns_and_falls_through_to_data_line():
    # Section 7.8.5: COMMENT must be followed by at least one space
...
def test_parse_kvn_mixed_case_keyword_warns_and_falls_through_to_data_line():
    # Section 7.4.4: keywords must be uppercase
...
def test_parse_kvn_line_exceeding_max_length_warns():
    # Section 7.3.2: line-length limit is opt-in via max_line_length
    long_line = "X = " + "1" * 300
...
def test_parse_kvn_no_max_line_length_never_warns_on_long_line():
    # Default (max_line_length=None) is for formats with no limit, e.g. OCM (section 7.3.3)
    long_line = "X = " + "1" * 300
...
def test_parse_kvn_non_printable_ascii_warns():
    # Section 7.3.4: only printable ASCII and blanks are permitted
