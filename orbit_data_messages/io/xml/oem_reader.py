...
class XMLOEMReader:
    ...
    def _parse(self, root: ET.Element) -> OEM:
        ...
        header: OEM.Header = OEM.Header(**read_model(
            header_element, OEM.Header,
        ...
        for segment_element in find_all(body_element, get_xml_tag(OEM.Segment)):
            metadata_element: ET.Element | None = find_child(segment_element, get_xml_tag(OEM.Segment.Metadata))
            ...
            state_vector_tag: str = get_xml_tag(OEM.Segment.EphemerisData.EphemerisDataLine)
            covariance_matrix_tag: str = get_xml_tag(OEM.Segment.CovarianceMatrix.CovarianceMatrixLines)
            ...
            phase: str = "ephem"   # 'ephem' -> before stateVectors; 'cov' -> between SV/COV
            ...
            state_vector_lines: list[OEM.Segment.EphemerisData.EphemerisDataLine] = [
                OEM.Segment.EphemerisData.EphemerisDataLine(
                    **read_model(state_vector_element, OEM.Segment.EphemerisData.EphemerisDataLine)
                )
            ...
            covariance_matrix_lines: OEM.Segment.CovarianceMatrix.CovarianceMatrixLines = OEM.Segment.CovarianceMatrix.CovarianceMatrixLines
            covariance_matrix_lines_list: list[OEM.Segment.CovarianceMatrix.CovarianceMatrixLines] = [
                covariance_matrix_lines(**read_model(covariance_matrix_element, covariance_matrix_lines))
                for covariance_matrix_element in find_all(data_element, covariance_matrix_tag)
            ]
            covariance_matrix: OEM.Segment.CovarianceMatrix | None = (
                ...
                if covariance_matrix_lines_list else None
            )
