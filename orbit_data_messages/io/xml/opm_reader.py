...
from orbit_data_messages.io.xml.parser import parse_xml_string
...
class XMLOPMReader:
    ...
    def _parse(
        ...
        segment_element: ET.Element | None = find_child(find_child(root, _TAG_BODY), _TAG_SEGMENT)
        metadata_element: ET.Element | None = find_child(segment_element, get_xml_tag(OPM.Metadata))
        data_element: ET.Element | None = find_child(segment_element, get_xml_tag(OPM.Data))

        header: OPM.Header = OPM.Header(**read_model(
            header_element, OPM.Header,
            extra_kvs={"CCSDS_OPM_VERS": version},
        ))
        metadata: OPM.Metadata = OPM.Metadata(**read_model(metadata_element, OPM.Metadata))

        state_vector_element: ET.Element | None = find_child(data_element, get_xml_tag(OPM.Data.StateVector))
        state_vector: OPM.Data.StateVector = OPM.Data.StateVector(**read_model(state_vector_element, OPM.Data.StateVector))

        osculating_keplerian_elements_element: ET.Element | None = find_child(data_element, get_xml_tag(OPM.Data.OsculatingKeplerianElements))
        keplerian: OPM.Data.OsculatingKeplerianElements | None = (
            OPM.Data.OsculatingKeplerianElements(
                **read_model(osculating_keplerian_elements_element, OPM.Data.OsculatingKeplerianElements)
            )
            if osculating_keplerian_elements_element is not None else None
        )

        spacecraft_parameters_element: ET.Element | None = find_child(data_element, get_xml_tag(OPM.Data.SpacecraftParameters))
        spacecraft: OPM.Data.SpacecraftParameters | None = (
            OPM.Data.SpacecraftParameters(**read_model(spacecraft_parameters_element, OPM.Data.SpacecraftParameters))
            if spacecraft_parameters_element is not None else None
        )

        covariance_matrix_element: ET.Element | None = find_child(data_element, get_xml_tag(OPM.Data.CovarianceMatrix))
        cov: OPM.Data.CovarianceMatrix | None = (
            OPM.Data.CovarianceMatrix(**read_model(covariance_matrix_element, OPM.Data.CovarianceMatrix))
            if covariance_matrix_element is not None else None
        )

        maneuver_parameters_elements: list[ET.Element] = find_all(data_element, get_xml_tag(OPM.Data.ManeuverParameters))
        maneuvers: list[OPM.Data.ManeuverParameters] | None = (
            [OPM.Data.ManeuverParameters(**read_model(element, OPM.Data.ManeuverParameters))
             for element in maneuver_parameters_elements]
            or None
        )

        user_defined_parameters_element: ET.Element | None = find_child(data_element, get_xml_tag(OPM.Data.UserDefinedParameters))
        user: OPM.Data.UserDefinedParameters | None = (
            OPM.Data.UserDefinedParameters(**read_model(user_defined_parameters_element, OPM.Data.UserDefinedParameters))
            if user_defined_parameters_element is not None else None
        )
