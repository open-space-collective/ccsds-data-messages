...
class XMLOPMWriter:
    ...
    def _build(
        ...
        *,
        ...
        root: ET.Element = ET.Element(get_xml_tag(OPM))
        ...
        write_model(message.header, header_element, skip_fields=frozenset({"ccsds_opm_vers"}), options=options)

        body_element: ET.Element = ET.SubElement(root, _TAG_BODY)
        segment_element: ET.Element = ET.SubElement(body_element, _TAG_SEGMENT)
        metadata_element: ET.Element = ET.SubElement(segment_element, get_xml_tag(OPM.Metadata))
        ...
        state_vector_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OPM.Data.StateVector))
        ...
        if message.data.osculating_keplerian_elements is not None:
            osculating_keplerian_elements_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OPM.Data.OsculatingKeplerianElements))
            write_model(message.data.osculating_keplerian_elements, osculating_keplerian_elements_element, options=options)

        if message.data.spacecraft_parameters is not None:
            spacecraft_parameters_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OPM.Data.SpacecraftParameters))
            write_model(message.data.spacecraft_parameters, spacecraft_parameters_element, options=options)

        if message.data.covariance_matrix is not None:
            covariance_matrix_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OPM.Data.CovarianceMatrix))
            write_model(message.data.covariance_matrix, covariance_matrix_element, options=options)

        if message.data.maneuvers:
            for maneuver in message.data.maneuvers:
                maneuver_parameters_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OPM.Data.ManeuverParameters))
                write_model(maneuver, maneuver_parameters_element, options=options)

        if message.data.user_defined is not None:
            user_defined_parameters_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OPM.Data.UserDefinedParameters))
            write_model(message.data.user_defined, user_defined_parameters_element, options=options)
