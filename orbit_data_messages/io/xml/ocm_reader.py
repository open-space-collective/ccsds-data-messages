...
def _read_block_with_lines(
    ...
    kwargs["data_lines"] = [get_text(line_element) for line_element in find_all(element, get_xml_line_tag(model_class))]
    return kwargs
...
class XMLOCMReader:
    ...
    def _parse(
        ...
        metadata_element: ET.Element | None = find_child(segment_element, get_xml_tag(OCM.Metadata))
        data_element: ET.Element | None = find_child(segment_element, _TAG_DATA)

        header: OCM.Header = OCM.Header(**read_model(
            header_element, OCM.Header,
            extra_kvs={"CCSDS_OCM_VERS": version},
        ))
        metadata: OCM.Metadata = OCM.Metadata(**read_model(metadata_element, OCM.Metadata))
        ...
        trajectory_states: list[OCM.TrajectoryStateTimeHistory] | None = (
            [OCM.TrajectoryStateTimeHistory(
             ...
             for element in find_all(data_element, get_xml_tag(OCM.TrajectoryStateTimeHistory))]
            or None
        )

        phys_element = find_child(data_element, get_xml_tag(OCM.SpaceObjectPhysicalCharacteristics))
        physical_characteristics: OCM.SpaceObjectPhysicalCharacteristics | None = (
            ...
            if phys_element is not None else None
        )
        ...
        pert_element: ET.Element | None = find_child(data_element, get_xml_tag(OCM.PerturbationsSpecification))
        perturbations: OCM.PerturbationsSpecification | None = (
            OCM.PerturbationsSpecification(**read_model(pert_element, OCM.PerturbationsSpecification))
            if pert_element is not None else None
        )

        od_element: ET.Element | None = find_child(data_element, get_xml_tag(OCM.OrbitDeterminationData))
        orbit_determination: OCM.OrbitDeterminationData | None = (
            OCM.OrbitDeterminationData(**read_model(od_element, OCM.OrbitDeterminationData))
            if od_element is not None else None
        )

        user_element: ET.Element | None = find_child(data_element, get_xml_tag(OCM.UserDefinedParameters))
        user_defined: OCM.UserDefinedParameters | None = (
            OCM.UserDefinedParameters(**read_model(user_element, OCM.UserDefinedParameters))
            if user_element is not None else None
        )
