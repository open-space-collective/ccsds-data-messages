...
class XMLOMMReader:
    ...
    def _parse(self, root: ET.Element) -> OMM:
        ...
        metadata_element: ET.Element | None = find_child(segment_element, get_xml_tag(OMM.Metadata))
        data_element: ET.Element | None = find_child(segment_element, get_xml_tag(OMM.Data))

        header: OMM.Header = OMM.Header(**read_model(
            header_element, OMM.Header,
            extra_kvs={"CCSDS_OMM_VERS": version},
        ))
        metadata: OMM.Metadata = OMM.Metadata(**read_model(metadata_element, OMM.Metadata))

        mean_keplerian_elements_element: ET.Element | None = find_child(data_element, get_xml_tag(OMM.Data.MeanKeplerianElements))
        ...
        spacecraft_parameters_element: ET.Element | None = find_child(data_element, get_xml_tag(OMM.Data.SpacecraftParameters))
        ...
        tle_related_parameters_element: ET.Element | None = find_child(data_element, get_xml_tag(OMM.Data.TLERelatedParameters))
        tle_related_parameters: OMM.Data.TLERelatedParameters | None = (
            OMM.Data.TLERelatedParameters(
                **read_model(tle_related_parameters_element, OMM.Data.TLERelatedParameters)
            )
            if tle_related_parameters_element is not None else None
        )

        covariance_matrix_element: ET.Element | None = find_child(data_element, get_xml_tag(OMM.Data.CovarianceMatrix))
        covariance_matrix: OMM.Data.CovarianceMatrix | None = (
            OMM.Data.CovarianceMatrix(**read_model(covariance_matrix_element, OMM.Data.CovarianceMatrix))
            if covariance_matrix_element is not None else None
        )

        user_defined_parameters_element: ET.Element | None = find_child(data_element, get_xml_tag(OMM.Data.UserDefinedParameters))
        user_defined_parameters: OMM.Data.UserDefinedParameters | None = (
            OMM.Data.UserDefinedParameters(
                **read_model(user_defined_parameters_element, OMM.Data.UserDefinedParameters)
            )
