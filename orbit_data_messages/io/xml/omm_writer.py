...
class XMLOMMWriter:
    ...
    def _build(
        ...
        *,
        ...
        metadata_element: ET.Element = ET.SubElement(segment_element, get_xml_tag(OMM.Metadata))
        ...
        mean_keplerian_elements_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OMM.Data.MeanKeplerianElements))
        write_model(message.data.mean_keplerian_elements, mean_keplerian_elements_element, options=options)

        if message.data.spacecraft_parameters is not None:
            spacecraft_parameters_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OMM.Data.SpacecraftParameters))
        ...
        if message.data.tle_related_parameters is not None:
            tle_related_parameters_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OMM.Data.TLERelatedParameters))
            write_model(message.data.tle_related_parameters, tle_related_parameters_element, options=options)

        if message.data.covariance_matrix is not None:
            covariance_matrix_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OMM.Data.CovarianceMatrix))
        ...
        if message.data.user_defined is not None:
            user_defined_parameters_element: ET.Element = ET.SubElement(data_element, get_xml_tag(OMM.Data.UserDefinedParameters))
