...
_TLE_THEORIES: frozenset[MeanElementTheory] = frozenset({MeanElementTheory.SGP, MeanElementTheory.SGP4})
...
class OMM(CCSDSDataMessage, BaseModel):
        ...
        mean_element_theory: Annotated[
            ...
            Field(
                description=(
                    ...
                    "Select from: SGP, SGP4, SGP4-XP, DSST, USM, PPT3 (§4.2.3, table 4-2)."
                ),
            ),
    ...
    class Data(BaseModel):
        ...
        class MeanKeplerianElements(BaseModel):
            ...
            mean_element_theory is SGP or SGP4 (4.2.4.6); cross-block
        ...
        class TLERelatedParameters(BaseModel):
            """
            Required when MEAN_ELEMENT_THEORY=SGP or SGP4 (4.2.4.6).
        ...
        tle_related_parameters: TLERelatedParameters | None = Field(
            ...
            description=(
                ...
                "MEAN_ELEMENT_THEORY=SGP or SGP4 (4.2.4.6)."
            ),
        )
    ...
    def check_tle_block_required(self) -> "OMM":
        """Validate TLE block is present when MEAN_ELEMENT_THEORY is SGP or SGP4 (4.2.4.6)."""
        theory: MeanElementTheory = self.metadata.mean_element_theory
    ...
    def check_norad_cat_id_required(self) -> "OMM":
        """NORAD_CAT_ID is required when MEAN_ELEMENT_THEORY is SGP or SGP4 (4.2.4.6)."""
        theory: MeanElementTheory = self.metadata.mean_element_theory
    ...
    def check_mean_motion_required_for_tle(self) -> "OMM":
        """Validate MEAN_MOTION is used (not SEMI_MAJOR_AXIS) for SGP/SGP4 theories (4.2.4.6)."""
        theory: MeanElementTheory = self.metadata.mean_element_theory
    ...
    def check_tle_drag_params(self) -> "OMM":
        ...
        if theory == MeanElementTheory.SGP4:
        ...
        if theory == MeanElementTheory.SGP:
            if tle.mean_motion_dot is None:
                raise ValueError(
                    ...
                    "mean_element_theory='SGP' (4.2.4.7)."
                )
    ...
    def check_teme_requires_tle_theory(self) -> "OMM":
        if self.metadata.ref_frame == RefFrame.TEME:
            if self.metadata.mean_element_theory not in _TLE_THEORIES:
                raise ValueError(
                    "REF_FRAME=TEME may only be used for OMMs based on NORAD TLE sets "
                    "(MEAN_ELEMENT_THEORY must be SGP or SGP4 per 4.2.4.9)."
                )
        return self
