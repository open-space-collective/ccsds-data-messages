...
# A relative time tag is a signed or unsigned decimal number (seconds).
# The spec (6.2.2.3) describes it as a double-precision floating-point value.
...
def _validate_time_tag(v: str, field_name: str) -> str:
    """Accept either an absolute CCSDS date or a relative time (seconds)."""
...
class OCM(CCSDSDataMessage, BaseModel):
    ...
    class Metadata(BaseModel):
        ...
        object_type: Annotated[
            ...
            Field(
                default=None,
                description=(
                    "Type of object. Select from the accepted set of values in "
                    "annex B, subsection B11 "
        ...
        ops_status: Annotated[
            ...
            Field(
                ...
                description=(
                    ...
                    "Examples: OPERATIONAL, NONOPERATIONAL, PARTIALLY OPERATIONAL, "
                    "BACKUP/STANDBY, SPARE, EXTENDED MISSION, DECAYED, UNKNOWN."
                ),
            ),
        ...
        @classmethod
        def validate_optional_dates(cls, v: str | None, info) -> str | None:
            if v is not None:
        ...
        @classmethod
        def validate_optional_time_tags(cls, v: str | None, info) -> str | None:
            if v is not None:
    ...
    class TrajectoryStateBlock(BaseModel):
        """
        ...
        center_name: Annotated[
            ...
            Field(
                description=(
                    ...
                    "(from annex B, subsection B2) or another reference frame center such as "
                    "a spacecraft or formation flying reference. Default EARTH."
                ),
            ),
        ...
        traj_type: Annotated[
            ...
            Field(
                description=(
                    "Trajectory state type. Selected from SANA Registry of Orbital Elements "
                    "(annex B, subsection B7, https://sanaregistry.org/r/orbital_elements). "
                    "Examples: CARTP, CARTPV, CARTPVA, KEPLERIAN, EQUINOCTIAL. Default CARTPV."
                ),
            ),
        ...
        @classmethod
        def validate_useable_times(cls, v: str | None, info) -> str | None:
            if v is not None:
                return _validate_time_tag(v, info.field_name)
            return v
        ...
        @model_validator(mode="after")
        def check_interpolation_degree_required(self) -> "OCM.TrajectoryStateBlock":
            if (
                self.interpolation is not None
                and self.interpolation.upper() != "PROPAGATE"
        ...
        @model_validator(mode="after")
        def check_orb_revnum_basis_required(self) -> "OCM.TrajectoryStateBlock":
    ...
    class PhysicalPropertiesBlock(BaseModel):
        """
        ...
        @model_validator(mode="after")
        def check_oeb_parent_frame_required(self) -> "OCM.PhysicalPropertiesBlock":
            quaternion_fields = (self.oeb_q1, self.oeb_q2, self.oeb_q3, self.oeb_qc)
            any_quaternion = any(q is not None for q in quaternion_fields)
    ...
    class CovarianceBlock(BaseModel):
        """
        ...
        cov_type: Annotated[
            ...
            Field(
                description=(
                    ...
                    "Examples: CARTP, CARTPV, CARTPVA, ADBARV. Default CARTPV."
                ),
            ),
        ...
        @field_validator("cov_ordering")
        @classmethod
        def validate_cov_ordering(cls, v: str) -> str:
            allowed = {"LTM", "UTM", "FULL", "LTMWCC", "UTMWCC"}
            if v.upper() not in allowed:
                raise ValueError(
                    f"cov_ordering must be one of {sorted(allowed)}, got '{v}'."
                )
            return v.upper()
    ...
    class ManeuverBlock(BaseModel):
        ...
        man_basis: Annotated[
            ...
            Field(
                ...
                description=(
                    ...
                    "Values: CANDIDATE, PLANNED, ANTICIPATED, TELEMETRY, "
        ...
        grav_assist_name: Annotated[
            ...
            Field(
                ...
                description=(
                    ...
                    "Select from annex B, subsection B2 (CENTER_NAME values)."
                ),
            ),
        ...
        @field_validator("dc_type")
        @classmethod
        def validate_dc_type(cls, v: str) -> str:
            allowed = {"CONTINUOUS", "TIME", "TIME_AND_ANGLE"}
            if v.upper() not in allowed:
                raise ValueError(
                    f"dc_type must be one of {sorted(allowed)}, got '{v}'."
                )
            return v.upper()
        ...
        def validate_man_composition_has_one_time(cls, v: str) -> str:
            elements = [e.strip().upper() for e in v.split(",")]
            time_specs = [e for e in elements if e in ("TIME_ABSOLUTE", "TIME_RELATIVE")]
            if len(time_specs) != 1:
                raise ValueError(
            ...
            _EXCLUSIVE_6_8 = _TABLE_6_8 - _TABLE_6_9
            _EXCLUSIVE_6_9 = _TABLE_6_9 - _TABLE_6_8
            has_6_8 = any(e in _EXCLUSIVE_6_8 for e in elements)
            has_6_9 = any(e in _EXCLUSIVE_6_9 for e in elements)
            ...
            order = _ORDER_6_9 if has_6_9 else _ORDER_6_8
            pos_map = {kw: i for i, kw in enumerate(order)}
            positions = [pos_map[e] for e in elements if e in pos_map]
        ...
        @model_validator(mode="after")
        def check_dc_time_fields_required(self) -> "OCM.ManeuverBlock":
        ...
        @model_validator(mode="after")
        def check_dc_angle_fields_required(self) -> "OCM.ManeuverBlock":
        ...
        @model_validator(mode="after")
        def check_pulse_period_ge_duration(self) -> "OCM.ManeuverBlock":
            if (
    ...
    class PerturbationsBlock(BaseModel):
        """
        ...
        shadow_model: Annotated[
            ...
            Field(
                ...
                description=(
                    ...
                    "One of: NONE, CYLINDRICAL, CONE, DUAL_CONE."
                ),
            ),
        ...
        @field_validator("shadow_model")
        @classmethod
        def validate_shadow_model(cls, v: str | None) -> str | None:
            if v is not None:
                allowed = {"NONE", "CYLINDRICAL", "CONE", "DUAL_CONE"}
                if v.upper() not in allowed:
                    raise ValueError(
                        f"shadow_model must be one of {sorted(allowed)}, got '{v}'."
                    )
                return v.upper()
            return v
    ...
    class OrbitDeterminationBlock(BaseModel):
        """
    ...
    trajectory_states: list[TrajectoryStateBlock] | None = Field(
        default=None,
        description=(
    ...
    physical_properties: PhysicalPropertiesBlock | None = Field(
        default=None,
        description=(
    ...
    covariances: list[CovarianceBlock] | None = Field(
        default=None,
        description=(
    ...
    maneuvers: list[ManeuverBlock] | None = Field(
        default=None,
        description=(
    ...
    perturbations: PerturbationsBlock | None = Field(
        default=None,
        description=(
    ...
    orbit_determination: OrbitDeterminationBlock | None = Field(
        default=None,
        description=(
