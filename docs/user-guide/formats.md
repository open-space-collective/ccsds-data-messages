...
Notation) and **XML**. Both are defined in section 7 (KVN) and section 8 (XML), and
...
```python
from orbit_data_messages import write, WriterOptions
write(msg, "output.xml", options=WriterOptions(include_units=False))
```

## Format detection

When no explicit `fmt=` is passed to `read()`, format is detected in this order:
